<template>
  <div  style="padding: 10px">
    ...
    <!--搜索-->
    <div style="margin: 10px 0">
      <el-input v-model="search" placeholder="请输入关键字" style="width: 20%" @clear="load" clearable></el-input>
<!--      <el-button type="primary" style="margin-left: 5px;" @click="load">查询</el-button>-->
      <el-button :icon="Search" style="margin-left: 5px;" @click="load" circle></el-button>
    </div>
    <el-table :data="tableData" border stripe style="width: 100%">
      <el-table-column prop="borrowId" label="借阅ID" sortable/>
    ...
    <el-dialog v-model="dialogVisible" title="书籍信息" width="30%">
      <el-form :model="form" label-width="120px">
        ...
        <el-form-item label="借阅日期">
          <el-date-picker value-format="YYYY-MM-DD" v-model="form.borrowDate" type="date" placeholder="请选择借阅的日期"/>
        </el-form-item>
        <el-form-item label="归还日期">
          <el-date-picker value-format="YYYY-MM-DD" v-model="form.returnDate" type="date" placeholder="请填写归还的日期"/>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods:{
    hasTitleNumber(){
      return this.form.bookName.includes("《")||this.form.bookName.includes("》")
    },
    examineExist(){
      request.get("/user/examineExist",this.form).then(res=>{
    ...
    handleDelete(borrowId){
      ...
      request.delete("/borrow/"+borrowId).then(res=>{
        this.option="删除"
        Global.methods.showMessage(res,this.option)
        this.load()
      })
